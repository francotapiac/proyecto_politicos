...
public class SentimentClassifier {
    ...
    public String classify(String tweet){
        ...
        for(int i=0;i<categorizer.getNumberOfCategories();i++){
            if(categorizer.getCategory(i).equals("positive")){
                if(prob[i] >= 0 && prob[i] < 0.25){
                    clasification = "very negative";
                }
                else if(prob[i] >= 0.25 && prob[i] < 0.5){
                    clasification = "negative";
                }
                else if(prob[i] >= 0.5 && prob[i] < 0.75){
                    clasification = "positive";
                }
                else if (prob[i] >= 0.75 && prob[i] <= 1.0){
                    clasification = "very positive";
                }
                else{
                    clasification = "unknown clasification";
                }
            }
        }
